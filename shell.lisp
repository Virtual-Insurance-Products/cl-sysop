...
;; This was the old way of sending commands to things
(defun -> (target command &rest options)
  ;; first we have to build up the arguments to the command method
  (let* ((>1-options '(:input :output :error :line-reader))
         (command-options (labels ((filtered-options (list)
                                     (when list
                                       (cond ((member (car list) >1-options)
                                              (filtered-options (cddr list)))
                                             ((eq (car list) :show)
                                              (filtered-options (cdr list)))
                                             ;; just omit nils
                                             ((not (first list))
                                              (filtered-options (cdr list)))
                                             (t (cons (car list) (filtered-options (cdr list))))))))
                            (if (and (listp (first options))
                                     (keywordp (first (first options))))
                                (filtered-options (cdr options))
                                (filtered-options options))))
         (command-string (string-list (cons command
                                            (let ((after-cmd nil))
                                              (loop for a in command-options
                                                    collect (cond ((stringp a)
                                                                   (if after-cmd
                                                                       (progn (setf after-cmd nil)
                                                                              a)
                                                                       (escape-for-shell a)))
                                                                  ((eq a '\|)
                                                                   (setf after-cmd t)
                                                                  ...
                                                                  ((eq a '\;)
                                                                   ...
                                                                   ";")
                                                                  ((keywordp a)
                                                                   (s "-~A" (string-downcase (symbol-name a))))
                                                                  ((symbolp a)
                                                                   (symbol-name a))
                                                                  ((listp a)
                                                                   (if (second a)
                                                                       (s "~A~A" (first a) (second a))
                                                                       (s "~A" (first a))))
                                                                  (t (s "~A" a))))))
    ...
    (let ((exec (if target
                    (apply #'command (cons target
                                           (if (and (listp (first options))
                                                    (keywordp (first (first options))))
                                               (append (list command-string)
                                                       (first options))
                                               (list command-string))))
                    command-string)))
      (if (find :show options)
          exec
          (apply #'>1 (cons exec
                            (loop for opt in >1-options
                                  when (member opt options)
                                    collect opt
                                  when (member opt options)
                                    collect (second (member opt options)))))))))
